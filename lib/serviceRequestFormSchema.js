...
export const serviceRequestFormSchema = z
  .object({
    ...
    urgency: z.string().min(1, "Urgency required"),
    businessImpact: z.string().min(1, "Business impact required"),
    preferredVisit: z.string().min(1, "Preferred visit required"),
  ...
  .superRefine((data, ctx) => {
    ...
    if (
      isOther &&
      (!data.problemDescription || data.problemDescription.length < 1)
    ) {
      ctx.addIssue({
        code: z.ZodIssueCode.custom,
        message: "Problem description is required",
