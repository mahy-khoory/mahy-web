...
import { normalizeCountryRegionCode } from "../utils/country";
...
export const fetchCustomerGroups = async (company) => {
  const { data } = await client.get("api/terms/cust-groups", {
    params: { company },
  });
...
{
  const {data} = await client.get('api/terms/countries');
  return normalizeLookup(data,{
    value: "CountryRegion",
    label: "CountryRegion",
    description: "CountryShortName"
  })
};

export const fetchCities = async (country) => {
  const normalizedCountry = normalizeCountryRegionCode(country);

  if (!normalizedCountry) {
    return [];
  }

  const { data } = await client.get("api/terms/cities", {
  ...
  return normalizeLookup(data, {
    value: "Name",
    label: "Name",
    description: (row) => row.StateId || row.CountryShortName || "",
  });
};
