...
export const vendorFormSchema = z.object({
    ...
    vendorClassificationGroup: z.enum(["onetime", "regular"]).default("onetime"),
    ...
    if (isOrganization) {
        ...
        if (showMethodOfPayment && (!data.methodOfPayment || data.methodOfPayment.length < 1)) {
            ctx.addIssue({
                code: z.ZodIssueCode.custom,
    ...
    if (isPerson) {
        ...
        const hasLastNamePrefix = data.lastNamePrefix && data.lastNamePrefix.length > 0;
        const hasLastName = data.lastName && data.lastName.length > 0;
        const anyNameFilled = hasFirstName || hasMiddleName || hasLastNamePrefix || hasLastName;
