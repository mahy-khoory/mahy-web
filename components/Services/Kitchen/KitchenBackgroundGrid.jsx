...
function KitchenBackgroundGrid({ items, bgImage }) {
  return (
    <section
      className="md:h-screen w-full relative bg-slate-800 bg-cover bg-center"
      style={bgImage ? { backgroundImage: `url(${bgImage})` } : undefined}
    >
      <div className="absolute inset-0 bg-slate-800/0.25 z-10" />
      ...
      <div className="relative md:absolute inset-0 z-20 flex flex-col justify-between py-8 md:py-15 max-w-7xl mx-auto px-5">
        <div className="flex justify-center md:justify-between flex-wrap gap-3">
          <h2 className="t-base font-bold text-lg text-center md:text-start">
            How we work
          </h2>

          <p className="max-w-lg md:text-xl text-white text-center md:text-start">
            Our unique methodology is designed to empower organizations, driving
            transformative change and sustainable growth. Discover the key
            pillars of our collaborative process:
          </p>
        </div>
...
const Card = ({ item }) => (
  <div>
    ...
    <div className="bg-gray-900 p-6 md:p-8">
      ...
      <p className="text-right text-8xl font-semibold text-slate-800">
        {item.count}
      </p>
    </div>
  </div>
