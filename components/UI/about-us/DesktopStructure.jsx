...
function DesktopStructure({ items, itemVariants, containerVariants, bg = "bg-white", bgHover = "hover:bg-white" }) {
                ...
                {items.map((item, i) => (
                    <motion.div key={i} variants={itemVariants} initial="hidden" whileInView="visible" custom={i} viewport={{ once: true }}>
                        <button onClick={() => setCurrentIndex(i)}
                            className={`${i === currentIndex ? bg : `bg-gray-100 ${bgHover}`} py-12 px-4 w-full h-full rounded-2xl transition-all duration-500`}>
